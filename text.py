...
def repeat_text_with_emoji(text, count, emojis):
...
# Emoji set input
emoji_set = input("Enter the emojis to choose from (separated by spaces): ")
emojis = emoji_set.split()

# Generate repeated text with emojis
repeated_texts_with_emojis = []
for _ in range(input_count):
    repeated_text_with_emoji = repeat_text_with_emoji(input_text, 1, emojis)
    repeated_texts_with_emojis.append(repeated_text_with_emoji)

# Save texts to a file
filename = "repeated_texts.txt"
